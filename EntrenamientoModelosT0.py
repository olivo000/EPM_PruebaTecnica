...
class EarlyStoppingCallback:
    ...
    def __call__(self, study, trial):
        ...
        if current_best < self.best_value - self.min_delta:
            ...
            self.counter += 1
            print(f"⚠️ Trial {trial.number}: sin mejora. ({self.counter}/{self.patience})")
...
# ---------- Generar TARGETS automáticamente ----------
targets = []
...
# ---------- Asegurar mínimo de nodos ----------
MIN_REQUIRED_NODES = 5

if len(targets) < MIN_REQUIRED_NODES:
    print(f"⚠️ Solo se seleccionaron {len(targets)} nodos. Buscando nodos adicionales para cumplir mínimo de {MIN_REQUIRED_NODES}...")
...
for asset_id, hour, tx_type in targets:
    ...
    # ---------- Cálculo de fechas clave (CORREGIDO) ----------
    ...
    # Evita dependencia de la hora → series iguales para todas
    ...
    if na_cols:
        print(f"⚠️ Columnas con NaN en df0 para {asset_id} h{hour} {tx_type}:")
        for col in na_cols:
            count = df0[col].isna().sum()
            print(f"  → {col}: {count} NaNs ({count / len(df0):.2%})")

    # Mostrar si el valid_mask eliminó muchos
